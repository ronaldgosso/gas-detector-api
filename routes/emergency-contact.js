...
router.post('/', async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: { id: result.insertId || 'existing', phone_number, contact_name }
        });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Failed to save emergency contact', error: error.message });
    }
});
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: 'Contact not found or already deactivated' });
        }

        res.json({ success: true, message: 'Emergency contact deactivated successfully' });
    } catch (error) {
        console.error('Delete route error:', error);
        res.status(500).json({ success: false, message: 'Internal server error while deactivating contact' });
    }
});
...
router.get('/settings/sms-selection', async (req, res) => {
    try {
        ...
        if (contactId !== '0') {
            ...
            if (contact.length > 0) {
                return res.json({ success: true, data: contact[0], isMultiple: false });
            }
        }

        res.json({ success: true, data: null, isMultiple: true });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error fetching SMS selection', error: error.message });
    }
});
...
router.put('/settings/sms-selection', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'SMS selection updated successfully' });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error updating SMS selection', error: error.message });
    }
});
